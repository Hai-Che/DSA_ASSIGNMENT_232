                      ...
                      Dataset &X_train, Dataset &X_test, Dataset &Y_train, Dataset &Y_test)
{
    ...
    // cout << nRows << "\n";
    // cout << nCols << "\n";
    // cout << test_size * nRows << "\n";
    // cout << int(test_size * nRows) << "\n";
    X_train = X.extract(0, nRows - int(test_size * nRows) - 1 + int(flag), 0, -1);
    // X_train = X.extract(0, (1 - test_size) * xRows - 1, 0, -1);
    // cout << X.extract(0, nRows - int(test_size * nRows) - 1, 0, -1).getData()->get(0)->get(0);
    // cout << X_train.getData()->get(0)->get(0);

    Y_train = Y.extract(0, nRows - int(test_size * nRows) - 1 + int(flag), 0, 0);
    // Y_train = Y.extract(0, (1 - test_size) * yRows - 1, 0, 0);

    //* phân chia phần test

    X_test = X.extract(nRows - int(test_size * nRows) + int(flag), nRows, 0, -1);
    // X_test = X.extract((1 - test_size) * xRows, xRows, 0, -1);

    Y_test = Y.extract(nRows - int(test_size * nRows) + int(flag), nRows, 0, 0);
...
//     X.getShape(xRows, xCols);
//     Y.getShape(yRows, yCols);
//     bool flag1 = test_size * xRows == int(test_size * xRows);
//     bool flag2 = test_size * yRows == int(test_size * yRows);

//     X_train = X.extract(0, (1 - test_size) * xRows - 1 + int(flag1), 0, -1);

//     Y_train = Y.extract(0, (1 - test_size) * yRows - 1 + int(flag2), 0, -1);

//     //* phân chia phần test

//     X_test = X.extract((1 - test_size) * xRows + int(flag1), xRows, 0, -1);

//     Y_test = Y.extract((1 - test_size) * yRows + int(flag2), yRows, 0, -1);
// }
